...
//    private  final String forbiddenURL =  PropsUtil.getString(properties,"app.forbiddenURL");
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            // 获取并遍历 Action 映射
            Map<RequestBean, ControllerBean> controllerMap = UrlMapper.getMpper();
            for (Map.Entry<RequestBean, ControllerBean> actionEntry : controllerMap.entrySet()) {
                ...
                Matcher requestPathMatcher = Pattern.compile(requestRegPath).matcher(currentRequestPath);
                // 判断请求方法与请求路径是否同时匹配
                if (requestMethod.equalsIgnoreCase(currentRequestMethod) && requestPathMatcher.matches()) {
                    ...
                    for (int k = 0; k < paramsMap.size();k++) {
                    	...
                    	if(paramClass.isPrimitive())
                    	{
	                    		...
	                    		actionMethodParamList.add(castPirmitiveObject(paramClass,string));
                    	}
                    	else {
							if(paramClass==File.class)
							{
								actionMethodParamList.add(getFileFromRequest(request,paramName));
							}
                 ...
                 // 从 BeanHelper 中创建 Action 实例
                    Object controllerObject = BeanContainer.getBean(controllerClass);
                    // 调用 Action 方法
                    ActionResult actionResult;
                  
                        actionMethod.setAccessible(true); // 取消类型安全检测（可提高反射性能）
                        actionResult = (ActionResult)actionMethod.invoke(controllerObject, actionMethodParamList.toArray());
...
//        }
    ...
    private File getFileFromRequest(HttpServletRequest request,String fieldName) throws IOException
    {
    	if(WebContext.isFileUpload())
    	{
    		...
    		{
    			if(item.getFieldName().equals(fieldName)&&!item.isFormField())
    			{
    ...
    private Map<String,String> getRequstParamsMap(HttpServletRequest request) throws UnsupportedEncodingException
    {
